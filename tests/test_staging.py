...
from data_pipeline_core import Record, Source, raw_landing_sink, raw_landing_source
...
def test_roundtrip_through_raw_bucket(tmp_path: Path) -> None:
    ...
    source: Source[Record] = raw_landing_source("odds-raw", bucket_url=bucket)
    replayed = list(source.fetch(None))  # type: ignore[arg-type]
    assert sorted(replayed, key=lambda r: int(r["id"])) == [
        {"id": 1, "x": "a"},
        {"id": 2, "x": "b"},
    ]
...
def test_landing_source_has_a_name(tmp_path: Path) -> None:
    source: Source[Record] = raw_landing_source(
        "odds-raw", bucket_url=tmp_path.as_uri()
    )
